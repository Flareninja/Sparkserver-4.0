...
exports.BattleFormats = {
    ...
    littlecupcreationism: {
		...
		validateSet: function (set) {
            ...
            // Put the original Pokemon species in whitelist, and the pre-evolution in prevoName. Maintain order.
            whitelist = ['Qwilfish', 'Sawk', 'Cryogonal', 'Bouffalant', 'Stunfisk'];
            prevoName = ['Qwilfry', 'Stryke', 'Hexaflake', 'Buffalad', 'Funfisk'];
            
            for (var i = 0; i < whitelist.length; i++) {
                if (set.species === whitelist[i]) {
                    set.name = prevoName[i];
                }
            }
			if (!template.nfe && (whitelist.indexOf(set.species) < 0)) {
				return [set.species + " doesn't have an evolution family. If this Pokemon has an approved pre-evolution for this project, it hasn't been implemented on the server yet. Please contact sparktrain, nineage, or fender."];
			}
		}
	},
	speciesclause: {
